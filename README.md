...
> **本分支是小红书小工具适配版**（从 `main` 派生）。玩法、规则、AI 强度与 `main`
> 完全一致，只改了运行环境相关的部分：移除 Web Worker、经典脚本产物、Chrome 61
> 兼容基线、竖屏适配、离线 zip 打包。改动清单与验收结论见
> [小红书小工具适配说明.md](./小红书小工具适配说明.md)。
> 打包命令：`npm run build:xhs`。
>
> 网页原版请看 [`main`](../../tree/main) 分支，带在线对战的完整版在
> [`online-mode`](../../tree/online-mode) 分支。
